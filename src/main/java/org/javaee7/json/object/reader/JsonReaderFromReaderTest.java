...
@WebServlet(urlPatterns = "/JsonReaderFromReaderTest")
public class JsonReaderFromReaderTest extends HttpServlet
{
    private static final long serialVersionUID = 1L;
    private final String servletName = "JsonReaderFromReaderTest";
    PrintWriter out;
    ...
    public void testEmptyObject() throws Exception {
        ...
        out.println("json was empty as expected");
    }
    ...
    public void testArray() throws Exception {
        ...
        if(jsonArr == null)
        	throw new Exception("Json should not be null but it was");
        if(2 != jsonArr.size())
        	throw new Exception("Json size should be 2 but it was: " + jsonArr.size());

        String expectedJson = "{\"apple\":\"red\"}";
        if(!expectedJson.equals(jsonArr.get(0).toString()))
        	throw new Exception("JsonArr should have been " + expectedJson + " but instead it was " + jsonArr.get(0));
        
        expectedJson = "{\"banana\":\"yellow\"}";
        if(!expectedJson.equals(jsonArr.get(1).toString()))
        	throw new Exception("JsonArr should have been " + expectedJson + " but instead it was " + jsonArr.get(1));
    ...
    public void testNestedStructure() throws Exception {
        ...
        if(!json.containsKey("title"))
        	throw new Exception("Json should contain 'title' but it did not: " + json);
        if(!"The Matrix".equals(json.getString("title")))
        	throw new Exception("Json should contain title=The Matrix but it did not: " + json);
        if(!json.containsKey("year"))
        	throw new Exception("Json should contain 'year' but it did not: " + json);
        if(1999 != json.getInt("year"))
        	throw new Exception("Json year should have year=1999 but it did not: " + json);
        if(!json.containsKey("cast"))
        	throw new Exception("Json should contain key 'cast' but it did not: " + json);
        ...
        if(!expectedJson.equals(jsonArr.toString()))
        	throw new Exception("JsonArr should have been " + expectedJson + " but instead it was " + jsonArr);
